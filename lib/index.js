exports.react = function(props, ReactWidget, reply){

  props.domId = about.name + '-' + Math.round( Math.random() * 1000 );
  props.className =  about.name;
  ...
  reply(
    [ myAppHtml,
     '<script>',
      ...
      camelCase(about.name) + '('+ JSON.stringify(props) +', "' + props.domId + '")',
     '</script>'
    ].join('\n')
  )
...
exports.relative_urls = function(req, fallback_proxy_path, urls) {
  ...
  Object.keys(urls).forEach(function(key){
    rel[key] = '//' + api_host + '/' + widgetless_name + urls[key];
  })
...
exports.load_settings = function(settings_db, query, host, cb){
  ...
  try {
    ...
    widget_token.load_settings(settings_db, settings_id, host, function(err, settings){
      ...
      _.assign(_secure, settings.general, settings.per_host)
      cb(null, settings_merge);
    })
  } catch(e){ return cb({}) }
}
